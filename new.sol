...
contract TRDCvault is BEP20 {
  ...
  modifier setPower(address _Player){
    ...
    constructor (uint _number, string memory _guessThisToo) BEP20("TRDC-Game-V1", "|$|") payable{
    ...
    function editBigBank(uint bankIndex, string memory _bankName, BEP20 _currency, uint _bankPower, uint _bankVault) external onlyOwner{
        ...
        bigBank[bankIndex].vaultAmount = 
    }
    function deleteBigBank(uint bankIndex) external onlyOwner{
        bigBank[bankIndex] = bigBank[bigBank.length -1];
        bigBank.pop();
    }
    function addBank (string memory _bankName,uint _bankPower, uint _bankVault) external onlyOwner{
    ...
    function addThief (string memory _tName, uint _tPower) external onlyOwner{
    ...
    function addCop (string memory _cName, uint _cPower) external onlyOwner{
